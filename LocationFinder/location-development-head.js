...
function checkRetention() {
  let expirationDate =
    new Date(ipLocationCache.timeStamp).getTime() + retentionFactor;
...
function findClosestLocations(currentLocation, placeSearchingThisAreaPin) {
  ...
  let numberOfLocations =
    locationList.childNodes.length < 10 ? locationList.childNodes.length : 10;
  ...
  let sortedLocations = locations.sort((loc1, loc2) =>
    loc1.distance < loc2.distance ? -1 : loc1.distance > loc2.distance ? 1 : 0
  );
  ...
  for (i = 0; i < listItems.length; i++) {
    try {
      document
        .querySelectorAll('[locname="' + sortedLocations[i].name + '"]')[0]
        .setAttribute("ordernumber", i + 1);
    } catch (error) {
      console.error(error);
    }
  }
...
async function getIPLocationData() {
  ...
  if (checkForIPLocationCache()) {
    ...
    findClosestLocations(
      { lat: ipLocationCache.lat, lng: ipLocationCache.lng },
      true
    );
    createMapBounds();
  } else {
    ...
    ipRegistryPromise
      .then(async (locationData) => {
        ipLocationCache = {
          timeStamp: new Date(
            locationData.time_zone.current_time
          ).toUTCString(),
      ...
      .catch(async (err) => {
        ...
        await initMap();
        findClosestLocations(
          { lat: defaultCenterPosition.lat, lng: defaultCenterPosition.lng },
          true
        );
        createMapBounds();
      });
